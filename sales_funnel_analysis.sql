SELECT * FROM `salesfunnelanalysis-sql.sql_projects.sales_funnel` LIMIT 1000
...
WITH funnel_stages AS (
  ...
  FROM `salesfunnelanalysis-sql.sql_projects.sales_funnel`

  WHERE event_date >= TIMESTAMP_SUB(
    (SELECT MAX(event_date) FROM `salesfunnelanalysis-sql.sql_projects.sales_funnel`),
    INTERVAL 30 DAY)
)
...
SELECT 
  ...
  ROUND (stage_5_purchase * 100 / stage_1_views) AS overall_coversion_rate,
...
SELECT
  ...
  -- SAVE_DIVIDE - in case denominator = 0, it can return NULL instead of cause error and crashing report
FROM funnel_revenue
